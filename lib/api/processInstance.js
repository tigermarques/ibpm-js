 ...
 * @property {string} snapshotName - Snapshot name that this instance belongs to
 ...
 * @property {boolean} messageSent - `true` if the message was sent successfully and `false` otherwise
 */

/**
 * @typedef {object} RuntimeErrorFailedOperation
 * @property {string} instanceId - Instance internal ID
 ...
 * @property {string} instanceId - Instance internal ID
 * @property {string} taskId - Task internal ID
 * @property {string} errorMessage - Error message
 * @property {string} errorCode - Error code
 * @property {string} javaTrace - Java trace
 * @property {string} jsTrace - JS trace
 * @property {string} twTrace - TW trace
 * @property {date} timestamp - Error date
 * @property {string} branchId - Branch internal ID
 * @property {string} snapshotId - Snapshot internal ID
 */

/**
 * @typedef {object} RuntimeErrorResponse
 * @property {Array<RuntimeErrorFailedOperation>} failedOperations - List of failed operations
 * @property {Array<RuntimeError>} runtimeErrors - List of runtime errors
 */
...
/**
 * @typedef {object} ProcessInstanceAPIResponse - Inherits `status` and `message` from {@link APIResponse} and overrides the `data` property
 * @augments APIResponse
 * @property {InstanceDetails} data - instance details
 */

/**
 * @typedef {object} ProcessInstanceSendMessageAPIResponse - Inherits `status` and `message` from {@link APIResponse} and overrides the `data` property
 * @augments APIResponse
 * @property {MessageResponse} data - message status
 */
...
const instanceSuccessHandler = response => {
...
module.exports = {
  getById: (config, instanceId) => new Promise((resolve, reject) => {
    request.get(`${config.restUrl}/process/${instanceId}?parts=all`,
      buildConfig(config), buildResponseHandler(resolve, reject))
  }).then(instanceSuccessHandler).catch(error => {
  ...
  suspend: (config, instanceId) => commonChangeInstanceState(config, 'suspend', instanceId).then(instanceSuccessHandler).catch(error => {
    if (error.data && error.data.Data) {
      error.data = {
        errorNumber: error.data.Data.errorNumber,
        errorMessage: error.data.Data.errorMessage
  ...
  resume: (config, instanceId) => commonChangeInstanceState(config, 'resume', instanceId).then(instanceSuccessHandler).catch(error => {
    if (error.data && error.data.Data) {
      error.data = {
        errorNumber: error.data.Data.errorNumber,
        errorMessage: error.data.Data.errorMessage
  ...
  terminate: (config, instanceId) => commonChangeInstanceState(config, 'terminate', instanceId).then(instanceSuccessHandler).catch(error => {
    if (error.data && error.data.Data) {
      error.data = {
        errorNumber: error.data.Data.errorNumber,
        errorMessage: error.data.Data.errorMessage
  ...
  retry: (config, instanceId) => commonChangeInstanceState(config, 'retry', instanceId).then(instanceSuccessHandler).catch(error => {
    if (error.data && error.data.Data) {
      error.data = {
        errorNumber: error.data.Data.errorNumber,
        errorMessage: error.data.Data.errorMessage
